...
const PrivateMoviesScreenUI: React.FC<Props> = ({ navigation }) => {
  ...
  const getmovies = async () => {
    ...
    try {
      ...
      const moviesData = responseBody.map((document: { id: string, genre: string; name: string; rating: any; age: string }) => ({
        id: document.id,
    ...
    for (let count = 0; count < moviesData.length; count++) {
      ...
      elements.push(
        <Center marginBottom="4">
          <CardMovie movieID={movie.id} movieName={movie.name} movieAge={movie.age} movieRating={movie.rating} />
        </Center>);
    }
    return elements;
  };
    ...
    <VStack space={3} alignItems="center" height="100%">
      ...
      <Center>
        <ScrollView maxH="500">
          <VStack space={3} alignItems="center" height="100%">
            <Center marginBottom="4">
              <CardMovie movieID="1" movieName="Oppenheimer" movieAge="+16" movieRating="5" />
            </Center>
            <Center marginBottom="4">
              <CardMovie movieID="1" movieName="Interstellar" movieAge="+13" movieRating="3" />
            </Center>
            <Center marginBottom="4">
              <CardMovie movieID="1" movieName="Inception" movieAge="+13" movieRating="5" />
            </Center>
          </VStack>
        </ScrollView>
      </Center>
